...
void menu(char key, State *current_state)
{
    ...
    if (key == 4)
    {
        if (s_flag == 0)
        {
            if (m_ind == 0)
            {
                *current_state = e_viewlog;
                clcd_write(0x01, 0);
                return;
            }
            else if (m_ind == 1)
            {
                *current_state = e_downloadlog;
                clcd_write(0x01, 0);
                return;
            }
            else if (m_ind == 2)
            {
                *current_state = e_clearlog;
                clcd_write(0x01, 0);
                return;
            }
        }
    ...
    clcd_print(menu[m_ind], (0x80 + (1)));
    clcd_print(menu[m_ind + 1], (0xC0 + (1)));
}
